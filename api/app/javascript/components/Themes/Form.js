...
export default function Form ({ id, locale }) {
  ...
  useEffect(() => {
    const _fetch = async () => {
      const { data: { values, dictionaries } } = await axios.get(id ? path('edit_theme_path', { id, format: 'json' }) : path('new_theme_path', { format: 'json' }))
              ...
              <div className={form.input}>
                <select name="state" onChange={handleChange} value={values.state}>
                  {dictionaries.states.map(state =>
                    <option key={state} value={state}>{I18n.t(`theme.states.${state}`)}</option>
                  )}
                </select>
              </div>
            </label>
          ...
          <div>
            ...
            {!send &&
              <>
                ...
                {id &&
                  <a href={path('theme_path', { id })} onClick={handleDestroy} className={buttons.destroy}>Удалить</a>
                }
              </>
            }
          </div>
