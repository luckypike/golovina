            ...
            <form className={classNames(form.root, styles.form)} onSubmit={handleSubmit}>
              ...
              {((isRussia() && values.delivery_option) || isInternational() || isPickup()) && price &&
                <h2>
                  {I18n.t('cart.total')}
                  <div className={styles.price}>
                    <Price sell={parseFloat(price.sell) + (isInternational() ? (parseFloat(price.sell) > 45000 ? 0 : 2800) : (isRussia() && !isPromo() ? city[values.delivery_option] : 0)) + (values.gift ? price.gift : 0)} />
                  </div>
                </h2>
              }
